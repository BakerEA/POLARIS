...
def logit(input_filename, output_filename, covar_filename):
    ...
    gene_loc=[]
    gene_loc.append(start_gene)
    gene_loc.append(end_gene)
    gene_loc=np.unique(gene_loc)
    ...
    for line in f:
        datalist=[]
        testlist=line.split()
        for j in range(len(gene_loc)):
            datalist.append(testlist[(gene_loc[j])+2])
        prs.append(datalist)
    f.close()
    ...
    results=pd.DataFrame(index=range(end_gene-start_gene+1), columns=range(8))

    for gene in range(len(gene_loc)):
